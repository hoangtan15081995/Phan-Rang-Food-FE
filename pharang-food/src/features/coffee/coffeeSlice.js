...
const initialState = {
  coffeeList: [
    { name: "Cà phê Gia Phú", catagory: "Cơm", price: "30.000", address: "30 Ngô Gia Tự thành phố Phan Rang Tháp Chàm", rating: "4", image: "https://nld.mediacdn.vn/2020/9/19/intro-1594766282-1600504778592231123885.jpg" },
...
const slice = createSlice({
  ...
  initialState,
  reducers: {
    setCoffeeSuccess(state, action) {
      state.coffeeList = action.payload;
    },
  },
});
export const setCoffee = (coffee) => async (dispatch) => {
  try {
    dispatch(slice.actions.setCoffeeSuccess(coffee));
  } catch (error) {
    console.log(error);
  }
};
