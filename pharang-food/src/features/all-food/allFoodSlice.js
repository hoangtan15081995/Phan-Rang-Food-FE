...
const slice = createSlice({
  ...
  initialState,
  reducers: {
    setAllFoodSuccess(state, action) {
      state.allFoodList = action.payload;
    },
  },
});
export const setAllFood = (allFood) => async (dispatch) => {
  try {
    dispatch(slice.actions.setAllFoodSuccess(allFood));
  } catch (error) {
    console.log(error);
  }
};
