...
const slice = createSlice({
  ...
  initialState,
  reducers: {
    setPorridgeSuccess(state, action) {
      state.porridgeList = action.payload;
    },
  },
});
export const setPorridge = (Porridge) => async (dispatch) => {
  try {
    dispatch(slice.actions.setPorridgeSuccess(Porridge));
  } catch (error) {
    console.log(error);
  }
};
